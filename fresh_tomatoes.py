    ...
    <script type="text/javascript" charset="utf-8">
        ...
        $(document).ready(function () {
          $('.tile').hide().first().show("fast", function showNext() {
            // Only animate tiles, i.e., not headings
            $(this).nextAll('.tile').first().show("fast", showNext);
          });
        });
    </script>
</head>
...
# A single feature entry html template
tile_content = '''
<div class="col-md-6 col-lg-4 tile text-center" data-trailer-youtube-id="{trailer_youtube_id}" data-toggle="modal" data-target="#trailer">
    <img src="{poster_image_url}" width="220" height="342">
    <h2>{title}</h2>
    <p>{type_specific_content}</p>
</div>
'''
...
def create_tiles_content(features):
    ...
    for feature in features:
        ...
        youtube_id_match = re.search(
            r'(?<=v=)[^&#]+', feature.trailer_youtube_url)
        youtube_id_match = youtube_id_match or re.search(
            r'(?<=be/)[^&#]+', feature.trailer_youtube_url)
